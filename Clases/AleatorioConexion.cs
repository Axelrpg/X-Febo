...
{
    internal class AleatorioConexion
    {
        public void mostrarTabla(DataGridView tablaAleatorio)
        {
            try
            {
                Conexion con = new Conexion();
                String query = "select * from aleatorio order by id desc";

                tablaAleatorio.DataSource = null;

                MySqlDataAdapter adapter = new MySqlDataAdapter(query, con.setConnection());
                DataTable dataTable = new DataTable();
                adapter.Fill(dataTable);

                tablaAleatorio.DataSource = dataTable;
        ...
        public void añadirFila(String nombre, String tiempo)
        {
            ...
            {
                Conexion con = new Conexion();
                String query = "insert into aleatorio (nombre, tiempo)" +
